...
The qualitative structure is in very close agreement with the reference result in the paper (cf.\ the plots for $z_b$ in the numerical results section of \cite{ClasonKunisch2009}). The optimal control concentrates on a thin square ring-like set around the target region and exhibits pronounced localization, while the state attains an almost constant profile inside the square and transitions smoothly to (near) zero outside due to elliptic smoothing.
...
\section{An alternative route: variational discretization}
...
measure spaces with sparse solutions} (Casas--Clason--Kunisch). We will call it SparseFEM.
...
%------------------------------------------------------------
\subsection{Main idea: discretize the state, and sparsity becomes nodal}
...
%------------------------------------------------------------
\subsection{A canonical Dirac projection $\Lambda_h$}
...
``Dirac-at-nodes'' controls are not a heuristic—they are the \emph{canonical representatives} of equivalence
...
%------------------------------------------------------------
\subsection{Optimality structure and sparsity}
