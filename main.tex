...
measure spaces with sparse solutions} (Casas--Clason--Kunisch). :contentReference[oaicite:1]{index=1}
...
\end{equation}
where $a(\cdot,\cdot)$ is the bilinear form of $-\Delta + c_0$. :contentReference[oaicite:4]{index=4}
...
\]
and computation reduces to the finite vector $(\bar\lambda_1,\dots,\bar\lambda_{N(h)})$. :contentReference[oaicite:9]{index=9}
...
This is again amenable to a locally superlinearly convergent semismooth Newton method in finite dimension. :contentReference[oaicite:13]{index=13}
...
$\chi_{\{|p|>\alpha\}}$. :contentReference[oaicite:16]{index=16}
...
The predual route motivates \emph{why} one should solve for $p$ in a Hilbert space rather than discretizing measures directly. :contentReference[oaicite:20]{index=20}
...
\subsection{Semismooth Newton method for the regularized predual problem (measure case)}
\label{sec:ssn}
...
%------------------------------------------------------------
\subsubsection{Algorithm (paper version, with brief intuition)}
