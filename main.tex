...
\title{Elliptic Optimal Control with Measures - Seminar Report}
...
Using Stampacchia-type estimates and a duality argument (Littman--Stampacchia--Weinberger \cite{littman_stampacchia_weinberger_1963}),
...
Let $y_n:=y(u_n)$.
By the elliptic well-posedness in the very weak setting (Section~\ref{sec:pde_measures}),
...
(e.g.\ $\mathcal{V}=H^2(\Omega)\cap H^1_0(\Omega)$).
...
w = A^*p.
\]
In particular, $\|w\|_{L^2}=\|A^*p\|_{L^2}$ and $\langle z,w\rangle=\langle z,A^*p\rangle$, the saddle formulation \eqref{eq:saddle_start} can be rewritten as
...
Next, let $w$ (equivalently $p$) be arbitrary and fixed  in \eqref{eq:inf_sup_step3}. The dependence on $u$ is
...
\sup_{p\in H^2(\Omega)\cap H^1_0(\Omega)}\
\Bigl[
-\frac12\|A^*p\|_{L^2(\Omega)}^2
-\langle z,A^*p\rangle_{L^2(\Omega)}
...
\begin{theorem}[Existence and uniqueness of the predual minimizer]
\label{thm:predual_existence_measure}
Assume that $n\le 3$, that
\[
H^2(\Omega)\cap H_0^1(\Omega) \hookrightarrow C_0(\Omega)
\]
continuously, and that $\|A^*\cdot\|_{L^2(\Omega)}$ is an equivalent norm on
$H^2(\Omega)\cap H_0^1(\Omega)$.
Then the problem
\begin{equation}
\label{eq:predual_problem_measure}
\inf_{p\in H^2(\Omega)\cap H_0^1(\Omega)}
\left[
\frac12\|A^*p+z\|_{L^2(\Omega)}^2-\frac12\|z\|_{L^2(\Omega)}^2
\right]
\quad\text{subject to}\quad
\|p\|_{C_0(\Omega)}\le \alpha
\end{equation}
admits a unique minimizer $p^*\in H^2(\Omega)\cap H_0^1(\Omega)$.
\end{theorem}
...
\qquad
J(p) := \frac12\|A^*p+z\|_{L^2(\Omega)}^2-\frac12\|z\|_{L^2(\Omega)}^2,
\]
and define the admissible set
\[
K_\alpha := \{p\in H : \|p\|_{C_0(\Omega)}\le \alpha\}.
\]

\medskip
\noindent\textbf{Step 1: The feasible set is nonempty and we choose a minimizing sequence.}
Since $0\in K_\alpha$, the admissible set is nonempty. Let $(p_n)\subset K_\alpha$ be a minimizing sequence for \eqref{eq:predual_problem_measure}, that is,
\[
J(p_n)\to \inf_{p\in K_\alpha} J(p).
\]
Because $0$ is feasible, we have
\[
\inf_{p\in K_\alpha} J(p)\le J(0)=0.
\]
Hence, after discarding finitely many terms if necessary, we may assume that
\[
J(p_n)\le 1
\qquad\text{for all }n.
\]
...
\medskip
\noindent\textbf{Step 2: The minimizing sequence is bounded in $H$.}
From $J(p_n)\le 1$ we obtain
\[
\frac12\|A^*p_n+z\|_{L^2(\Omega)}^2-\frac12\|z\|_{L^2(\Omega)}^2 \le 1,
\]
hence
\[
\|A^*p_n+z\|_{L^2(\Omega)}^2 \le \|z\|_{L^2(\Omega)}^2 + 2.
\]
Therefore,
\[
\|A^*p_n+z\|_{L^2(\Omega)}
\le \sqrt{\|z\|_{L^2(\Omega)}^2+2}.
\]
By the triangle inequality,
\[
\|A^*p_n\|_{L^2(\Omega)}
\le \|A^*p_n+z\|_{L^2(\Omega)} + \|z\|_{L^2(\Omega)}
\le \sqrt{\|z\|_{L^2(\Omega)}^2+2} + \|z\|_{L^2(\Omega)}.
\]
Thus $(A^*p_n)$ is bounded in $L^2(\Omega)$. Since $\|A^*\cdot\|_{L^2(\Omega)}$ is an equivalent norm on $H$, it follows that $(p_n)$ is bounded in $H$.

\medskip
\noindent\textbf{Step 3: Extract a weakly convergent subsequence.}
Since $H$ is a Hilbert space, every bounded sequence admits a weakly convergent subsequence. Hence there exist a subsequence, not relabeled, and an element $p^*\in H$ such that
\[
p_n \rightharpoonup p^*
\qquad\text{weakly in }H.
\]

\medskip
\noindent\textbf{Step 4: The constraint is preserved under weak convergence.}
We now show that the weak limit $p^*$ is still feasible.

Because the embedding $H\hookrightarrow C_0(\Omega)$ is continuous, the map
\[
p\mapsto \|p\|_{C_0(\Omega)}
\]
is convex and continuous on $H$. Therefore
\[
K_\alpha=\{p\in H:\|p\|_{C_0(\Omega)}\le \alpha\}
\]
is a closed convex subset of $H$. Since $H$ is a Hilbert space, every closed convex subset is weakly closed. As each $p_n$ belongs to $K_\alpha$ and $p_n\rightharpoonup p^*$ in $H$, we conclude that
\[
p^*\in K_\alpha.
\]
Hence the weak limit is again admissible.

\medskip
\noindent\textbf{Step 5: Weak lower semicontinuity yields existence of a minimizer.}
The map
\[
p\mapsto A^*p+z
\]
is affine and continuous from $H$ to $L^2(\Omega)$, and the functional
\[
v\mapsto \frac12\|v\|_{L^2(\Omega)}^2
\]
is convex and continuous on $L^2(\Omega)$. It follows that $J$ is weakly lower semicontinuous on $H$. Therefore,
\[
J(p^*)
\le \liminf_{n\to\infty} J(p_n)
= \inf_{p\in K_\alpha} J(p).
\]
Since $p^*\in K_\alpha$, this shows that $p^*$ is a minimizer of \eqref{eq:predual_problem_measure}.

\medskip
\noindent\textbf{Step 6: The minimizer is unique.}
The admissible set $K_\alpha$ is convex. Moreover, the functional
\[
p\mapsto \frac12\|A^*p+z\|_{L^2(\Omega)}^2
\]
is strictly convex on $H$. Indeed, the map
\[
v\mapsto \frac12\|v+z\|_{L^2(\Omega)}^2
\]
is strictly convex on $L^2(\Omega)$, and the norm equivalence assumption implies that $A^*$ is injective on $H$. Thus, if $p_1\neq p_2$, then $A^*p_1\neq A^*p_2$, and for every $\theta\in(0,1)$,
\[
J(\theta p_1+(1-\theta)p_2)
<
\theta J(p_1)+(1-\theta)J(p_2).
\]
Hence two distinct minimizers cannot exist. Therefore the minimizer $p^*$ is unique.
\end{proof}

\begin{remark}[Slater condition]
For the box constraint $K=\{p\in H_0^2(\Omega):\|p\|_\infty\le \alpha\}$ with $\alpha>0$,
Slater condition holds trivially (take $p\equiv 0$, so $\|p\|_\infty=0<\alpha$).
This constraint qualification is often invoked to justify strong duality and the existence
of Lagrange multipliers (which, due to $(C_0(\Omega)^*)=\mathcal M(\Omega)$, can be represented
as measures).
\end{remark}
...
condition for the $u$-subproblem \eqref{eq:u-subproblem}.

\paragraph{Step 1: The $u$-subproblem and its first-order condition.}
If we fix $p \in C_0(\Omega)$ in \ref{eq:inf_sup_step3} we encounter the convex minimization problem
...
Comparing \eqref{eq:kkt_inclusion} with \eqref{eq:E2_normal_cone_again}, we see that the
...
The derivation of the predual and the optimality conditions can be summarized in the following theorem proved by \cite{ClasonKunisch2009} in a different fashion.
\begin{theorem}[Clason--Kunisch]\label{thm:clason_thm_2_4}
...
\begin{remark}
      One might notice that in our derivation we have used the saddle-point derivation and the convex optimality condition for a comparison, which is a different approach from \cite{ClasonKunisch2009}. There is no particular motivation for this choice of strategy of derivation, except for an attempt to give another view.
\end{remark}
...
\begin{remark}
      We skipped the proof as the theorem is not a particular result of optimal control. The interested reader can find it in \cite{Bogachev2007MeasureTheory}.
\end{remark}
...
Let $\psi \in C_0(\Omega)$ with $p \ge 0$. Then:
\begin{align*}
&\langle u^*,p\rangle=0
\quad\text{if }\operatorname{supp}\psi\subset\{x:\ |p^*(x)|<\alpha\},
\\
...
but $p^{*\prime\prime\prime}$ has a jump there. We can use the following lemma from the theory of distributions.
...
\le K,
\]
for some constant $K$ independent of $c$ (here we use the norm equivalence assumption for $A^\ast$ on $H^2_0(\Omega)$).
...
Together with the weak convergence $p_c\rightharpoonup \tilde p$ in $H^2_0$ and the norm equivalence induced by $A^\ast$, this implies $p_c\to\tilde p$ strongly in $H^2_0(\Omega)$.
...
By uniqueness of the solution of the KKT system, we conclude $\tilde p=p^\ast$ and $\tilde\lambda=\lambda^\ast$.
...
Fix a Moreau--Yosida parameter $c>0$ and recall the regularized optimality system
...
\paragraph{Semismoothness and Newton derivative.}
...
\mathcal{A}_k^+ := \{x\in\Omega:\ p_k(x)>\alpha\},\qquad
\mathcal{A}_k^- := \{x\in\Omega:\ p_k(x)<-\alpha\},\qquad
\mathcal{A}_k := \mathcal{A}_k^+\cup \mathcal{A}_k^-.
\end{equation}
Then $\chi_{\{|p_k|>\alpha\}}=\chi_{\mathcal{A}_k}$ and \eqref{eq:ssn_step_abstract} becomes
\[
AA^\ast(p_{k+1}-p_k)+c(p_{k+1}-p_k)\chi_{\mathcal{A}_k} = -\Bigl(AA^\ast p_k + cP_\alpha(p_k)+Az\Bigr).
\]
After rearranging and using $P_\alpha(p_k)=(p_k-\alpha)\chi_{\mathcal{A}_k^+}+(p_k+\alpha)\chi_{\mathcal{A}_k^-}$,
one obtains the linear equation for $p_{k+1}$:
\begin{equation}\label{eq:ssn_step_strong}
\bigl(AA^\ast + c\,\chi_{\mathcal{A}_k}\bigr)p_{k+1}
=
-Az + c\alpha\,\chi_{\mathcal{A}_k^+} - c\alpha\,\chi_{\mathcal{A}_k^-}.
\end{equation}
...
+
c\langle p_{k+1}\chi_{\mathcal{A}_k},v\rangle_{L^2}
=
-\langle z, A^\ast v\rangle_{L^2}
+
c\alpha\langle \chi_{\mathcal{A}_k^+}-\chi_{\mathcal{A}_k^-},v\rangle_{L^2}
\end{equation}
...
\[
\mathcal{A}_k^+ := \{p_k>\alpha\},\qquad \mathcal{A}_k^- := \{p_k<-\alpha\},\qquad \mathcal{A}_k:=\mathcal{A}_k^+\cup \mathcal{A}_k^-.
\]
\item[(ii)] Compute $p_{k+1}\in H_0^2(\Omega)$ by solving \eqref{eq:ssn_step_weak}.
\item[(iii)] Set $k\leftarrow k+1$.
\end{enumerate}
\item Stop if $\mathcal{A}_k^+=\mathcal{A}_{k-1}^+$ and $\mathcal{A}_k^-=\mathcal{A}_{k-1}^-$.
\end{enumerate}
\end{algorithm}

\paragraph{Why the stopping criterion makes sense.}
If the active sets no longer change, the Newton derivative stays the same and the right-hand side in \eqref{eq:ssn_step_weak} is consistent with the nonlinear term.
...
If $\mathcal{A}_{k+1}^+=\mathcal{A}_k^+$ and $\mathcal{A}_{k+1}^-=\mathcal{A}_k^-$, then $p_{k+1}$ satisfies $F(p_{k+1})=0$, i.e.\ it solves \eqref{eq:opt_sys_reg}.
\end{proposition}

\begin{proof}
Fix the sets $\mathcal{A}_k^\pm$ and note that \eqref{eq:ssn_step_weak} has a unique solution for these fixed sets.
If $\mathcal{A}_{k+1}^\pm=\mathcal{A}_k^\pm$, then on $\mathcal{A}_{k+1}^+$ we have $p_{k+1}>\alpha$ and therefore
\[
c\,p_{k+1}\chi_{\mathcal{A}_{k+1}^+}-c\alpha\chi_{\mathcal{A}_{k+1}^+}=\max(0,c(p_{k+1}-\alpha)),
\]
and analogously on $\mathcal{A}_{k+1}^-$,
\[
c\,p_{k+1}\chi_{\mathcal{A}_{k+1}^-}+c\alpha\chi_{\mathcal{A}_{k+1}^-}=\min(0,c(p_{k+1}+\alpha)).
\]
...
We now establish the local convergence rate of the semismooth Newton scheme introduced above.
After proving existence and uniqueness of the solution $p_c$ to the regularized optimality system
\eqref{eq:opt_sys_reg}, the next natural question is how fast Algorithm~\ref{alg:ssn_paper}
approaches this solution. The following result shows that, provided the initial guess $p_0$
is chosen sufficiently close to $p_c$, the iterates are not merely convergent, but in fact
converge \emph{superlinearly} in $H_0^2(\Omega)$. In other words, once the iterates enter a
neighborhood of the solution, the error decreases faster than linearly, which is the characteristic
local behavior of semismooth Newton methods under a uniform invertibility condition for the
...
Fix a measurable set $\mathcal{A}\subset\Omega$ and
consider the linear operator
\[
L_\mathcal{A} : H^2_0(\Omega)\to H^2_0(\Omega)^*,
\qquad
\langle L_\mathcal{A}\phi,v\rangle
:=
\langle A^*\phi,A^*v\rangle_{L^2}
+
c\langle \chi_\mathcal{A}\phi,v\rangle_{L^2}.
\]
This is precisely $\partial_NF(p)$ with $A=\{|p|>\alpha\}$.
Assume that there exist constants $m,M>0$ such that
\[
m\|\phi\|_{H^2_0(\Omega)}
\le
\|A^*\phi\|_{L^2(\Omega)}
\le
M\|\phi\|_{H^2_0(\Omega)}
\qquad\forall\,\phi\in H^2_0(\Omega).
\]
Then the bilinear form
\[
a_\mathcal{A}(\phi,v):=\langle A^*\phi,A^*v\rangle_{L^2}+c\langle \chi_\mathcal{A}\phi,v\rangle_{L^2}
\]
is continuous on $H^2_0(\Omega)\times H^2_0(\Omega)$ and coercive with a coercivity constant independent of $\mathcal{A}$, because
\[
a_\mathcal{A}(\phi,\phi)
=
\|A^*\phi\|_{L^2(\Omega)}^2+c\|\chi_A\phi\|_{L^2(\Omega)}^2
\ge
\|A^*\phi\|_{L^2(\Omega)}^2
\ge
m^2\|\phi\|_{H^2_0(\Omega)}^2.
\]
Hence, by Lax--Milgram, for every $g\in H^2_0(\Omega)^*$ there exists a unique
$\phi\in H^2_0(\Omega)$ such that
\begin{equation}
\langle A^*\phi,A^*v\rangle_{L^2}
+
c\langle \chi_A\phi,v\rangle_{L^2}
=
\langle g,v\rangle_{H^2_0(\Omega)^*,H^2_0(\Omega)}
\qquad \forall v\in H^2_0(\Omega).
\label{eq:newton_linear_problem}
\end{equation}
Thus $L_\mathcal{A}$ is invertible.

\paragraph{Step 3: Uniform bound for the inverse.}
Coercivity of $a_\mathcal{A}$ yields the estimate
\[
\|\phi\|_{H_0^2(\Omega)} \le C \|g\|_{H_0^2(\Omega)^\ast},
\]
where $C$ depends only on $A$ (the PDE operator) and $\Omega$, but \emph{not} on $\mathcal{A}\subset\Omega$.
Equivalently,
\[
\|\mathcal{L}_\mathcal{A}^{-1}\|_{\mathcal{L}(H_0^2(\Omega)^\ast,H_0^2(\Omega))}\le C,
\]
uniformly over all active sets $\mathcal{A}$.

\paragraph{Step 4: Apply semismooth Newton theory.}
Taking $\mathcal{A}=\{|p|>\alpha\}$, the uniform inverse bound from Step~3 gives the required hypothesis in \cite[Thm.~8.16]{ItoKunisch2008}.
...
the update of $\mathcal{A}_k^\pm$ is the ``active set'' step, and \eqref{eq:ssn_step_weak} is the corresponding linearized KKT system.
The equivalence between both interpretations is classical; see \cite{HintermuellerItoKunisch2002}.

\section{Experiments}\label{sec:experiments}

\subsection{Experiment 1: Measure-valued control ($X=\mathcal{M}(\Omega)$)}\label{subsec:exp1_measures}

We begin with the benchmark example from the numerical section of \cite{ClasonKunisch2009}, where the elliptic operator is taken as
$A=-\Delta$ with homogeneous Dirichlet boundary conditions on the square domain
$\Omega=[-1,1]^2$.
The desired state is the discontinuous ``box'' target
\[
z_b(x,y)=\chi_{\{|x|<\tfrac12,\ |y|<\tfrac12\}},
\]
which is used in \cite{ClasonKunisch2009} to highlight the
structural effect of the measure control cost $\alpha\|u\|_{\mathcal{M}(\Omega)}$
(and, in particular, the sparsity effect described by the active-set characterization in Corollary~2.7 of \cite{ClasonKunisch2009}).

Following the approach of \cite{ClasonKunisch2009}, we solve the (regularized) predual box-constrained problem and recover the optimal control via the primal--predual relation.
Figure~\ref{fig:exp1_reproduction} shows the resulting target $z_b$, the computed optimal control $u^\ast$, and the corresponding state $y^\ast=A^{-1}u^\ast$.
The qualitative structure is in very close agreement with the reference result in \cite{ClasonKunisch2009} (see the plots for $z_b$ in the numerical results). The optimal control concentrates on a thin square ring-like set around the target region and exhibits pronounced localization, while the state attains an almost constant profile inside the square and transitions smoothly to (near) zero outside due to elliptic smoothing.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{result_basic_1.png}
    \caption{Experiment 1 (measure control): target $z_b$ (left), computed optimal control $u^\ast$ (middle), and resulting state $y^\ast=A^{-1}u^\ast$ (right).
    The obtained structure closely reproduces the qualitative behavior reported for the target $z_b$ in \cite{ClasonKunisch2009}.}
    \label{fig:exp1_reproduction}
\end{figure}

This localization is precisely the expected sparsity effect of the $\mathcal{M}(\Omega)$-control cost: $u^\ast$ is (essentially) supported where the predual variable hits the box constraints $\{|p^\ast|=\alpha\}$, and vanishes where the constraint is inactive $\{|p^\ast|<\alpha\}$ \cite{ClasonKunisch2009}, as we can see in Figure ~\ref{fig:exp2_reproduction}

\begin{figure}[th!]
    \centering
    \includegraphics[width=\linewidth]{result_basic_2.png}
    \caption{Experiment 1 (measure control): Solution of predual (left), Active set (positive) $p > \alpha$ (middle-left), Active set (negative) $p < \alpha$ (middle-right), and combination $p < \|\alpha\|$ (right).}
    \label{fig:exp2_reproduction}
\end{figure}

\subsection{Experiment 2: Semiconductor-inspired model with generation term}\label{subsec:exp2_semic}

We now apply the measure--space optimal control framework to a simplified design problem for \emph{photodiodes}, i.e.\ semiconductor devices that convert incident light into an electrical signal.
When light is absorbed in a semiconductor, it generates electron--hole pairs.
These charge carriers are transported to the electrodes through a combination of diffusion and drift (advection) driven by the electric field.
In many low-power and low-noise photodiode designs the internal electric field is deliberately kept weak, so that carrier transport is largely diffusion dominated.
This motivates, as a first modeling step, a diffusion-based description of carrier collection.

A photodiode is typically formed by bringing p-doped and n-doped semiconductor regions into contact, creating a p--n junction.
One key performance quantity is the junction capacitance, which (in first approximation) scales with the junction interface area.
Reducing capacitance is desirable because it improves bandwidth and reduces noise; however, a smaller junction area can also degrade carrier collection, since fewer generated carriers are swept or collected efficiently near the junction.
This leads to an intrinsic trade-off between \emph{capacitance minimization} (small junction area) and \emph{collection efficiency}.

We address this trade-off in an idealized geometry as follows.
We consider a fixed semiconductor domain $\Omega$ (e.g.\ uniformly n-doped as a background material), and we seek an optimal placement of doped regions that act as ``collectors''.
In our mathematical model, this placement is represented by a control variable $u$ that acts as a localized sink (or, more generally, as a measure-valued actuation) in the transport equation.
The governing PDE additionally contains a prescribed \emph{generation term} $g$ (e.g.\ optical generation), which accounts for the creation of electron--hole pairs by illumination and enters as an inhomogeneous source term in the state equation.
As discussed above, such a generation term can be incorporated into the framework by working with an inhomogeneous state equation of the form $A y = u + g$, i.e.\ the same control structure up to an affine shift of the state.

This can be incorporated into our framework without changing the underlying analysis: one simply replaces the homogeneous equation $A y = u$ by an inhomogeneous one
\begin{equation}\label{eq:state_with_generation}
A y \;=\; u + g,
\end{equation}
where $g$ denotes the known generation profile (typically localized in a subregion of $\Omega$), and $A$ is the same elliptic/parabolic operator as before (e.g.\ a diffusion operator, possibly with additional lower-order terms).
Equivalently, one may shift the state variable by introducing the particular solution $y_g := A^{-1}g$ and writing
\[
y \;=\; A^{-1}u + y_g .
\]
Hence the reduced objective becomes
\[
J(u) \;=\; \frac12\|A^{-1}u + y_g - z\|_{L^2(\Omega)}^2 \;+\; \alpha \|u\|_{\mathcal{M}(\Omega)}
\;=\; \frac12\|A^{-1}u - (z-y_g)\|_{L^2(\Omega)}^2 \;+\; \alpha \|u\|_{\mathcal{M}(\Omega)} ,
\]
which is exactly the same structure as in the previous experiment, with a \emph{shifted target} $\tilde z := z - y_g$.
In particular, the convexity, duality arguments, and the predual formulation carry over verbatim: the generation term influences the optimizer only through the affine shift $y_g$ (or, equivalently, through $\tilde z$), while the sparsity-promoting mechanism induced by the $\mathcal{M}(\Omega)$-term remains unchanged.

We solved the problem for diffusion coefficients equally spaced in the interval $[0.1,0.6]$. The corresponding optimal controls are shown in Figure~\ref{fig:exp3_reproduction}. A clear trend emerges: the optimal sink geometry adapts systematically to the diffusion strength, i.e., to how rapidly carriers spread across the domain. For small diffusivity, the optimizer favors highly localized, almost point-like sink patterns. This matches the intuition that when particles do not spread far, effective collection is achieved by concentrating the control at a few strategically placed locations. In contrast, for larger diffusivity the optimal control becomes increasingly distributed, forming an extended “net” that intercepts carriers over a wider area before they can reach (and be lost through) the boundary.

\begin{figure}[p!]
    \centering
    \includegraphics[width=\linewidth]{result_semic_3.png}
    \caption{Experiment 2 : Solution obtained for different diffusion coefficients for a constant illumination area (top left).}
    \label{fig:exp3_reproduction}
\end{figure}
...
Elliptic optimal control with measure-valued controls provides a natural framework for modeling \emph{localized actuation} and for enforcing \emph{sparsity} in PDE-constrained optimization. The main difficulty is that the control variable lives in the nonreflexive Banach space $\mathcal{M}(\Omega)$ and the objective contains the nonsmooth total variation term $\alpha\|u\|_{\mathcal{M}(\Omega)}$. As a consequence, neither standard $L^2$-control techniques nor classical smooth optimality systems apply directly. On the PDE side, measure right-hand sides also require a careful notion of solution, since a general measure does not belong to $H^{-1}(\Omega)$ and the usual $H_0^1$-weak formulation is not available.

The first part of the report resolves these issues analytically by working with the \emph{very weak} Dirichlet formulation and exploiting elliptic regularization. This yields a well-defined control-to-state mapping $S:\mathcal{M}(\Omega)\to L^2(\Omega)$ together with compactness properties that make the direct method applicable. In particular, boundedness in $\mathcal{M}(\Omega)$ gives weak-$\ast$ compactness (Banach--Alaoglu), while the associated states are compact in $L^2(\Omega)$ (Rellich), which allows one to prove existence (and under standard assumptions, uniqueness) of an optimal control. These mapping and compactness mechanisms are the core reason why the tracking term and the measure penalty can be combined into a well-posed optimization problem.

From a computational perspective, the key step is the \emph{predual reformulation} obtained by Fenchel duality: the nonsmooth measure norm is converted into the box constraint $\|p\|_\infty\le \alpha$ on a smooth variable $p\in H^2(\Omega)\cap H_0^1(\Omega)\hookrightarrow C_0(\Omega)$. This Hilbert-space formulation leads to a clear KKT system and, crucially, to a structural sparsity principle: the optimal measure $u^\ast$ is identified with (minus) the KKT multiplier and can only concentrate on the active set $\{|p^\ast|=\alpha\}$. To solve the resulting constrained problem efficiently, we then employ Moreau--Yosida regularization of the box constraint and a semismooth Newton / primal-dual active set strategy, supported by a convergence theorem showing that the regularized solutions converge to the original box-constrained solution as the regularization parameter tends to infinity.

Finally, we demonstrated how this framework can be applied to a semiconductor-inspired design problem for photodiodes in a diffusion-dominated regime. In this model, optical illumination enters as a prescribed generation term $g$, leading to an inhomogeneous state equation $Ay=u+g$ that fits into the same theory after an affine shift of the target. Numerically, the optimal sink patterns exhibit a physically meaningful dependence on the diffusion coefficient: for weak diffusion the optimal control concentrates into highly localized sink regions, while for stronger diffusion it becomes more distributed and forms an extended collection structure. This illustrates how measure-based sparsity provides an effective mathematical proxy for the design trade-off between localized collection (small active region, low “cost”) and global carrier removal (robust collection under strong spreading), thereby connecting the abstract optimal control theory to interpretable device-level behavior.
