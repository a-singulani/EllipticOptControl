...
Many optimization problems requires \textbf{localized actuation}, for example, heating spots, micro-heaters, point electrodes, concentrated loads. Thus, we need
\begin{itemize}  
...
\textbf{Elliptic smoothing for measure data}\newline
...
$\|u^\ast\|_{\M}\le\liminf_{n\to\infty}\|u_n\|_{\M}$, so $u^\ast$ is optimal.
...
\[
(\alpha\norm{\cdot}_\M)^\ast(\varphi) = \sup_{u\in\mathcal M(\Omega)}\{\langle u,p\rangle-\alpha\|u\|_{\mathcal M}\} 
=  I_{\{\|\phi\|_{X^*}\le \alpha\}}(p)
=
\begin{cases}
...
for $p\in C_0(\Omega)$, $p\ge0$,
\[
\supp(p)\subset\{x \in \Omega | |p^\ast(x)|<\alpha\}
\quad\Longrightarrow\quad
\langle u^\ast,p\rangle=0.
\]

\medskip
\textbf{Sign on the active set:}
\[
\supp(p)\subset\{x \in \Omega | p^\ast(x)=\alpha\}\Rightarrow \langle u^\ast,p\rangle\ge0,
\qquad
\supp(p)\subset\{x \in \Omega | p^\ast(x)=-\alpha\}\Rightarrow \langle u^\ast,p\rangle\le0.
\]
...
\begin{block}{Goal}
Solve the regularized optimality system $F(p)= AA^\ast p+Az+\max(0,c(p-\alpha))+\min(0,c(p+\alpha)) =0$ for $p\in H_0^2(\Omega)$
...
\vspace{0.6em}
\small
\textbf{Qualitative validation.}
To sanity-check our implementation, we compare the computed control/state patterns
against the benchmark examples for the square target $z_b$ in Clason--Kunisch (2009). 
\begin{itemize}\itemsep0.2em
\item the optimal control concentrates on a thin ring near the interface of the target set,
\item the resulting state reproduces the plateau inside the target with a smooth transition layer near the boundary,
\item corner effects and sign changes align with the qualitative patterns reported in the reference.
\end{itemize}
...
% ============================================================
%\begin{frame}{Alternative numerical route: variational discretization (SparseFEM)}
%\textbf{Different "philosophy"}
%Discretize only the \emph{state} equation by FEM, keep $u\in\M(\Om)$ continuous.

%\textbf{Key theorem (informal)}
